...
/// Middleware: return 401 for /api/* requests without a valid session (except public auth paths).
pub async fn require_auth_middleware(
    ...
    match state.db.get_session_user_id(&token).await {
...
async fn auth_setup(State(state): State<AppState>, Json(body): Json<AuthSetupBody>) -> Response {
    ...
    if let Err(e) = state.db.create_user(&id, username, &password_hash).await {
...
async fn auth_me(State(state): State<AppState>, jar: CookieJar) -> Response {
    ...
    Json(serde_json::json!({ "username": user.username, "id": user.id })).into_response()
}
