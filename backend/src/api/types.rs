...
pub struct AddProjectMemberRequest {
    pub user_id: String,
    /// member | viewer (project-level role)
...
pub struct UpdateApplicationRequest {
    ...
    /// Health check path for blue-green deploy, e.g. "/health". Default "health".
    pub health_path: Option<Option<String>>,
    /// Health check timeout in seconds. Default 5.
    pub health_timeout_secs: Option<Option<i32>>,
}
...
pub struct SetUserPermissionsRequest {
    /// List of permission keys, e.g. ["terminal", "manage_servers"]
...
pub struct CreateInviteRequest {
    ...
    /// admin | member | viewer
    pub role: Option<String>,
    /// Optional: send invite email if SMTP configured
